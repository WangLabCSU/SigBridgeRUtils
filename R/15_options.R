...
#' @rdname SigBridgeR_Function_Setting
#' @description
#' Shows all currently set SigBridgeR package options in a formatted display.
#' This function helps users inspect the current configuration settings.
#'
#' @param ... Additional arguments (currently not used)
#'
#' @return No return value, prints options to console
#'
#' @examples
#' \dontrun{
#' # Set some options first
#' setFunOption(verbose = TRUE, workers = 4)
#'
#' # Display all current SigBridgeR options
#' showFuncOption()
#' }
#'
#' @export
showFuncOption <- function(...) {
    options <- grep("^SigBridgeR\\.", names(options()), value = TRUE)
    if (length(options) == 0) {
        return(invisible())
    }
    cli::cli_div(
        theme = list(
            span.key = list(color = "white"),
            span.value = list(color = "blue"),
            span.type = list(color = "grey")
        )
    )
    for (opt_name in options) {
        cli::cli_bullets(c(
            " " = paste0(
                "{.key {opt_name}}: {.val {getOption(opt_name)}}"
            )
        ))
    }
    invisible()
